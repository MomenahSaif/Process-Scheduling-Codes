...
void multiFeedback(vector<Process>& processes) {
    cout << "Multi-Feedback Scheduling (q=1):" << endl << endl;
    ...
    while (true) {
        if (!queues[currentQueue].empty()) {
            ...
            int endTime = min(currentTime + 1, startTime + currentProcess.service - 1);
          ...
          currentProcess.Turnaround = (endTime - currentProcess.arrival);
          processes[currentProcess.id].Turnaround=currentProcess.Turnaround ;
